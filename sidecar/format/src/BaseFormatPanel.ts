...
const { mxEventObject, mxConstants, mxClient, mxPoint, mxEvent, mxGraph, mxUtils } = mx
...
export class BaseFormatPanel {
   ...
   */
  installInputHandler(input, key, defaultValue, min, max, unit, textEditFallback?, isFloat?) {
    unit = (unit != null) ? unit : '';
   ...
   * 
   */
  createStepper(input, update, step, height, disableFocus, defaultValue, isFloat) {
    ...
    if (disableFocus)
    {
      ...
      mxEvent.addGestureListeners(stepper, (evt) =>
        {
   ...
   */
  createCellOption(label, key, defaultValue, enabledValue, disabledValue, fn?, action?, stopEditing?) {
   ...
   * Adds the given color option.
   */
  createColorOption(label, getColorFn, setColorFn, defaultColor, listener, callbackFn?, hideCheckbox?) {
    var div = document.createElement('div');
    div.style.padding = '6px 0px 1px 0px';
    div.style.whiteSpace = 'nowrap';
    div.style.overflow = 'hidden';
   ...
   * 
   */
  createCellColorOption(label, colorKey, defaultColor, callbackFn, setStyleFn?) {
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;
    
    return this.createColorOption(label, () => {
      // Seems to be null sometimes, not sure why...
      var state = graph.view.getState(graph.getSelectionCell());
   ...
   * 
   */
  addUnitInput(container, unit, right, width, update, step?, marginTop?, disableFocus?, isFloat?) {
