...
export class TextFormatPanel extends BaseFormatPanel {
  ...
  addFont(container) {
    ...
    var verticalItem =
      this.editorUi.toolbar.addItems(["vertical"], stylePanel2, true)[0];
      ...
      ? this.createColorOption(
        ...
        function (color) {
          if (mxClient.IS_FF) {
            ...
            var newFonts = graph.cellEditor.textarea.getElementsByTagName(
              "font",
            );

            for (var i = 0; i < newFonts.length; i++) {
              if (
                i >= oldFonts.length || newFonts[i] != oldFonts[i].node ||
      ...
      container.appendChild(
        this.createRelativeOption(
          ...
          function (input) {
            ...
            while (
              node != null && node.nodeType != mxConstants.NODETYPE_ELEMENT
            ) {
              node = node.parentNode;
            }

            if (
              node != null && node == graph.cellEditor.textarea &&
            ...
            if (
              node != null && graph.cellEditor.textarea != null &&
    ...
    function setSelected(elt, selected) {
      if (
        mxClient.IS_IE && (mxClient.IS_QUIRKS || document.documentMode < 10)
      ) {
    ...
    var listener = (sender, evt, force) => {
      ...
      var fontStyle = mxUtils.getValue(
        ss.style,
      ...
      if (pos == mxConstants.ALIGN_LEFT && vpos == mxConstants.ALIGN_TOP) {
        ...
        pos == mxConstants.ALIGN_CENTER && vpos == mxConstants.ALIGN_TOP
      ) {
        positionSelect.value = "top";
      } else if (
        pos == mxConstants.ALIGN_RIGHT && vpos == mxConstants.ALIGN_TOP
      ) {
        positionSelect.value = "topRight";
      } else if (
        pos == mxConstants.ALIGN_LEFT && vpos == mxConstants.ALIGN_BOTTOM
      ) {
        positionSelect.value = "bottomLeft";
      } else if (
        pos == mxConstants.ALIGN_CENTER && vpos == mxConstants.ALIGN_BOTTOM
      ) {
        positionSelect.value = "bottom";
      } else if (
        pos == mxConstants.ALIGN_RIGHT && vpos == mxConstants.ALIGN_BOTTOM
      ) {
    ...
    if (graph.cellEditor.isContentEditing()) {
        ...
        if (!updating) {
          ...
          window.setTimeout(function () {
            ...
            if (node != null) {
              ...
              // IE does not support containsNode
              if (
                elts.length > 0 && window.getSelection && !mxClient.IS_IE &&
                !mxClient.IS_IE11
              ) {
                var selection = window.getSelection();
              ...
              if (css != null) {
                ...
                if (!graph.cellEditor.isTableSelected()) {
                  ...
                  setSelected(
                    center,
                    isEqualOrPrefixed(css.textAlign, "center"),
                  );
                ...
                tableCell = (currentTable == null)
                  ? null
                  : graph.getParentByNames(node, ["TD", "TH"], currentTable);
                tableWrapper.style.display = (currentTable != null)
                  ? ""
                  : "none";
      ...
      if (
        mxClient.IS_FF || mxClient.IS_EDGE || mxClient.IS_IE || mxClient.IS_IE11
      ) {
        mxEvent.addListener(
          graph.cellEditor.textarea,
          "DOMSubtreeModified",
