...
export class StyleFormatPanel extends BaseFormatPanel {
  ...
  init() {
    ...
    if (!ss.containsLabel) {
      if (
        ss.containsImage && ss.vertices.length == 1 &&
  ...
  addEditOps(div) {
    ...
    if (this.editorUi.editor.graph.getSelectionCount() == 1) {
      btn = mxUtils.button(mxResources.get("editStyle"), (evt) => {
  ...
  addFill(container) {
    ...
    gradientPanel.style.display =
      (tmpColor != null && tmpColor != mxConstants.NONE &&
        ss.fill && ss.style.shape != "image")
        ? ""
        : "none";
    ...
    var listener = () => {
      ...
      if (
        !ss.fill || ss.containsImage || fillColor == null ||
        fillColor == mxConstants.NONE || ss.style.shape == "filledEdge"
      ) {
  ...
  addStroke(container) {
    ...
    var addItem = mxUtils.bind(
      ...
      function (menu, width, cssName, keys, values) {
        ...
        pat.style.borderBottom = "1px " + cssName + " " +
    ...
    var edgeShape = this.editorUi.toolbar.addMenuFunctionInContainer(
      ...
      false,
      (menu) => {
        this.editorUi.menus.styleChange(
          menu,
          "",
          [
    ...
    var lineStart = this.editorUi.toolbar.addMenuFunctionInContainer(
      ...
      (menu) => {
        if (
          ss.style.shape == "connector" || ss.style.shape == "flexArrow" ||
  ...
  addStyleOps(div) {
    ...
    var btn = mxUtils.button(mxResources.get("setAsDefaultStyle"), (evt) => {
