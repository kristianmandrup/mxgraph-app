...
export class StaticOverrides {
    ...
    proto.validateBackgroundStyles = () => {
      ...
      var color =
        graph.background == null || graph.background == mxConstants.NONE
          ...
          : graph.background;
      var gridColor = color != null && proto.gridColor != color.toLowerCase()
        ? proto.gridColor
        : "#ffffff";
      var image = "none";
      var position = "";
  ...
  setCreateSvgGrid() {
    ...
    proto.createSvgGrid = function (color) {
      ...
      for (var i = 1; i < this.gridSteps; i++) {
        var tmp3 = i * tmp;
        const txt = "M 0 " + tmp3 + " L " + tmp2 + " " + tmp3 + " M " + tmp3 +
          " 0 L " + tmp3 + " " + tmp2;
        this.addToArr(d, txt);
      }
    ...
    proto.panGraph = (dx, dy) => {
      ...
      if (proto.shiftPreview1 != null) {
        ...
        var position = -Math.round(
          phase -
            mxUtils.mod(
              proto.view.translate.x * proto.view.scale + dx,
              phase,
            ),
        ) +
          "px " +
          -Math.round(
            phase -
              mxUtils.mod(
                proto.view.translate.y * proto.view.scale + dy,
                phase,
              ),
          ) +
          "px";
  ...
  setUpdatePageBreaks() {
    ...
    proto.updatePageBreaks = (visible, width, height) => {
      ...
      var drawPageBreaks = (breaks) => {
        if (breaks != null) {
          var count = breaks == proto.horizontalPageBreaks
            ? horizontalCount
            : verticalCount;

          for (var i = 0; i <= count; i++) {
            var pts = breaks == proto.horizontalPageBreaks
              ? [
                new mxPoint(
  ...
  setShouldRemoveCellsFromParent() {
    ...
    mxGraphHandler.prototype.shouldRemoveCellsFromParent = (
      ...
      return mxGraphHandlerShouldRemoveCellsFromParent.apply(
        this,
        [parent, cells, evt],
      );
    };
  ...
  setAddItem() {
    ...
    mxPopupMenu.prototype.addItem = (
      ...
      var result = mxPopupMenuAddItem.apply(
        this,
        [title, image, funct, parent, iconCls, enabled],
      );
