...
export class GraphCssTransformConfig {
  ...
  /**
	 ...
	 */
  isFastZoomEnabled() {
    return urlParams["zoom"] != "nocss" && !mxClient.NO_FO &&
      !mxClient.IS_EDGE &&
  ...
  getCellAt(x, y, parent, vertices, edges, ignoreFn) {
    ...
    return this.getScaledCellAt.apply(
      this,
      [x, y, parent, vertices, edges, ignoreFn],
    );
  }
    ...
    if (parent != null) {
      ...
      for (var i = childCount - 1; i >= 0; i--) {
        ...
        if (result != null) {
          ...
          this.isCellVisible(cell) && (edges && this.model.isEdge(cell) ||
            vertices && this.model.isVertex(cell))
        ) {
          var state = this.view.getState(cell);

          if (
            state != null && (ignoreFn == null || !ignoreFn(state, x, y)) &&
  ...
  isRecursiveVertexResize(state) {
    return !this.isSwimlane(state.cell) &&
