    ...
    this.graph.addMouseListener(
      {
        ...
        mouseUp: (sender, me) => {
          ...
          if (this.isResetEvent(evt)) {
            ...
            this.isActive() && !connectionHandlerActive &&
            this.mouseDownPoint != null
          ) {
            ...
            if (
              this.graph.getSelectionCount() != 1 || !this.graph.model.isEdge(
            ...
            mxEvent.isTouchEvent(evt) || (this.bbox != null &&
  ...
  drag(evt, x, y) {
    ...
    if (this.currentState != null) {
      ...
      if (
        ...
        mxEvent.isControlDown(evt) && es != null &&
 ...
 */
  click(state, dir, me) {
    ...
    if (
      tmp != null && this.graph.model.isEdge(tmp.cell) &&
    ...
    if (this.currentState != null) {
      ...
      if (
        ...
        positionArrow(
          this.arrowUp,
          Math.round(
            this.currentState.getCenterX() - this.triangleUp.width / 2 -
              this.tolerance,
          ),
        ...
        if (this.checkCollisions) {
          ...
          var checkCollision = (cell, arrow) => {
            var geo = this.graph.model.isVertex(cell) &&
 ...
 */
  update(state, x?, y?) {
    if (
      !this.graph.connectionArrowsEnabled || (state != null &&
        mxUtils.getValue(state.style, "allowArrows", "1") == "0")
    ) {
      this.reset();
    } else {
      if (
        state != null && state.cell.geometry != null &&
      ...
      if (this.prev != state || this.isActive()) {
        ...
        if (state != null) {
          ...
          this.updateThread = window.setTimeout(
            () => {
              if (
                !this.isActive() && !this.graph.isMouseDown &&
      ...
      if (
        ...
        this.currentState != null || timeOnTarget > this.activationDelay
      ) {
        if (
          this.currentState != state &&
          ((timeOnTarget > this.updateDelay && state != null) ||
            this.bbox == null || x == null || y == null ||
