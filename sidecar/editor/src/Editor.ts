...
export class Editor {
   ...
   * 
   */
  editBlankUrl: string = window.location.protocol + "//" +
  ...
  setup() {
    try {
      ...
      while (
        op.opener != null && typeof op.opener.Editor !== "undefined" &&
  ...
  getGraphXml(ignoreSelection) {
    ...
    if (ignoreSelection) {
      ...
      node = this.graph.encodeCells(
        mxUtils.sortCells(this.graph.model.getTopmostCells(
    ...
    node.setAttribute(
      "arrows",
      (this.graph.connectionArrowsEnabled) ? "1" : "0",
    );
