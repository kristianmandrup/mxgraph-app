...
const { STENCIL_PATH, IMAGE_PATH, urlParams } = resources;
      ...
      if (this.currentElt != elt) {
        ...
        var show = () => {
          ...
          if (w > this.maxTooltipWidth || h > this.maxTooltipHeight) {
            this.graph2.view.scale = Math.round(
          ...
          if (this.tooltipTitles && title != null && title.length > 0) {
            ...
            this.tooltipTitle != null && this.tooltipTitle.parentNode != null
          ) {
          ...
          var left = this.container.clientWidth + this.editorUi.splitSize + 3 +
            this.editorUi.container.offsetLeft + off.x;
          var top = Math.min(
            bottom - height - 20, /*status bar*/
            Math.max(
              0,
              (this.editorUi.container.offsetTop +
                this.container.offsetTop + elt.offsetTop -
                this.container.scrollTop - height / 2 + 16),
            ),
          ) + off.y;
