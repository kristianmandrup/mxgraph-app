...
const { MAX_REQUEST_SIZE, SAVE_URL } = resources;
/**
...
export class ExportDialog {
  constructor(editorUi) {
    ...
    transparentCheckbox.checked = graph.background == null ||
    ...
    function formatChanged() {
      ...
      if (
        imageFormatSelect.value === "png" || imageFormatSelect.value === "svg"
      ) {
    ...
    var saveBtn = mxUtils.button(
      ...
      () => {
        if (parseInt(zoomInput.value) <= 0) {
          ...
          if (
            (format == "svg" || format == "png") && transparentCheckbox.checked
          ) {
            bg = null;
