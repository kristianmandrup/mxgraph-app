...
export class PageSetupDialog {
  ...
  constructor(editorUi) {
    ...
    var accessor = PageSetupDialog.addPageFormatPanel(
      td, 'pagesetupdialog', graph.pageFormat, undefined);
    ...
    mxEvent.addListener(gridSizeInput, 'change', () => {
      ...
      this.gridSizeInput.value = Math.max(1, (isNaN(value)) ? graph.getGridSize() : value);
    });
...
/**
  ...
  static update(evt?, selectChanged?) {
    const { formatDiv, customDiv, widthInput, landscapeCheckBox, 
      pf, pageFormat, heightInput, paperSizeSelect, 
      customSize, currentPageFormat, pageFormatListener } = this
  ...
  static listener(sender?, evt?, force?) {
    const { widthInput, heightInput, formats, customSize, 
      paperSizeSelect, pageFormat, portraitCheckBox,
      landscapeCheckBox, formatDiv, customDiv } = this
