...
const { mxRectangle, mxPrintPreview, mxEvent, 
  mxUtils, mxResources, mxConstants, mxClient } = mx
...
export class PrintDialog {
  ...
  create(editorUi, title) {
    ...
    var printBtn = mxUtils.button(mxResources.get((!PrintDialog.previewEnabled) ? 'ok' : 'print'), () => {
      editorUi.hideDialog();
      preview(true);
    });
  ...
  preview(print) {
    const { graph, pageCountCheckBox, onePageCheckBox, 
      pageScaleInput, pageCountInput } = this
