...
const { mxEventObject, mxResources, mxConstants, mxClient, mxPoint, mxEvent, mxGraph, mxUtils } = mx
...
export class DiagramFormatPanel {
  ...
  addOptions(div) {
    ...
    if (graph.isEnabled()) {
      // Connection arrows
      div.appendChild(this.createOption(mxResources.get('connectionArrows'), () => {
      ...
      // Connection points
      div.appendChild(this.createOption(mxResources.get('connectionPoints'), () => {
      ...
      // Guides
      div.appendChild(this.createOption(mxResources.get('guides'), () => {
  ...
  addGridOption(container) {
    ...
    var stepper = this.createStepper(input, this.update, this.getUnitStep(), null, null, null, this.isFloatUnit());
    ...
    if (mxClient.IS_SVG) {
      ...
      var panel = this.createColorOption(mxResources.get('grid'), () => {
