...
import mx from "mx";
const { mxEventObject, mxConstants, mxClient, mxResources, mxPoint, mxEvent, mxGraph, mxUtils } = mx
...
export class StyleFormatPanel extends BaseFormatPanel {
  ...
  addSvgRule(container, rule, svg, styleElem, rules, ruleIndex, regex)
  {
    ...
    if (regex.test(rule.selectorText))
    {
      ...
      var addStyleRule = (rule, key, label) => {
        if (rule.style[key] != '')
        {
          var option = this.createColorOption(label + ' ' + rule.selectorText, () =>
          {
  ...
  addStroke(container)
  {
    ...
    var lineEnd = this.editorUi.toolbar.addMenuFunctionInContainer(stylePanel2, 'geSprite-endclassic', mxResources.get('lineend'), false, (menu) =>
    {
      if (ss.style.shape == 'connector' || ss.style.shape == 'flexArrow' || ss.style.shape == 'filledEdge')
      {
