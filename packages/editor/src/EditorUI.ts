import mx from "mx";
import { Menus } from "./menus/menus/manager/Menus";
import resources from "resources/resources";
import { ErrorDialog } from "sample/ErrorDialog";
 ...
 * screen widths <= 640px.
 */
  hsplitPosition = (screen.width <= 640)
    ? 118
 ...
 */
  ...
  hideDialog(cancel, isEsc?) {
    if (this.dialogs != null && this.dialogs.length > 0) {
      ...
      this.dialog = this.dialogs.length > 0
        ...
        if (Editor.useLocalStorage) {
          ...
          if (xml.length < MAX_REQUEST_SIZE) {
            const req = new mxXmlRequest(
              SAVE_URL,
              "filename=" + encodeURIComponent(name) + "&xml=" +
  ...
  setPageVisible(value) {
    ...
    if (hasScrollbars) {
      graph.container.scrollLeft = graph.view.translate.x * graph.view.scale -
        tx;
      graph.container.scrollTop = graph.view.translate.y * graph.view.scale -
        ty;
    }
