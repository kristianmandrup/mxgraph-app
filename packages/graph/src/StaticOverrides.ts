...
export class StaticOverrides {
      ...
      if (graph.container != null && !graph.transparentBackground) {
        if (graph.pageVisible) {
          ...
          if (this.backgroundPageShape == null) {
            ...
            if (firstChild != null) {
              this.backgroundPageShape = proto.createBackgroundPageShape(bounds)
    ...
    proto.validateBackgroundStyles = () => {
      ...
      var color =
        graph.background == null || graph.background == mxConstants.NONE
          ...
          : graph.background
      var gridColor =
        color != null && proto.gridColor != color.toLowerCase() ? proto.gridColor : '#ffffff'
      var image = 'none'
      var position = ''
      ...
      if (graph.isGridEnabled()) {
        ...
        position =
          -Math.round(phase - mxUtils.mod(proto.translate.x * proto.scale - x0, phase)) +
          'px ' +
          -Math.round(phase - mxUtils.mod(proto.translate.y * proto.scale - y0, phase)) +
          'px'
      }
  ...
  setPanGraph() {
    ...
    proto.panGraph = (dx, dy) => {
      ...
      if (proto.shiftPreview1 != null) {
        ...
        var position =
          -Math.round(phase - mxUtils.mod(proto.view.translate.x * proto.view.scale + dx, phase)) +
          'px ' +
          -Math.round(phase - mxUtils.mod(proto.view.translate.y * proto.view.scale + dy, phase)) +
          'px'
        canvas.style.backgroundPosition = position
      }
    }
  ...
  setUpdatePageBreaks() {
    ...
    proto.updatePageBreaks = (visible, width, height) => {
      ...
      visible = visible && Math.min(bounds.width, bounds.height) > proto.minPageBreakDist
      ...
      var drawPageBreaks = mxUtils.bind(this, function (breaks) {
        if (breaks != null) {
          var count = breaks == proto.horizontalPageBreaks ? horizontalCount : verticalCount
  ...
  setShouldRemoveCellsFromParent() {
    ...
    mxGraphHandler.prototype.shouldRemoveCellsFromParent = (parent, cells, evt) => {
      ...
      return mxGraphHandlerShouldRemoveCellsFromParent.apply(this, [parent, cells, evt])
    }
  }
  ...
  setCreateBackgroundPageShape() {
    ...
    proto.createBackgroundPageShape = (bounds) => {
      return new mxRectangleShape(bounds, '#ffffff', proto.graph.defaultPageBorderColor)
    }
  }
  ...
  setPanGraphVML() {
    ...
    mxGraph.prototype.panGraph = (dx, dy) => {
      ...
      if (
        ...
        (!proto.useScrollbarsForPanning || !mxUtils.hasScrollbars(proto.container))
      ) {
  ...
  setAddItem() {
    ...
    mxPopupMenu.prototype.addItem = (title, image, funct, parent, iconCls, enabled) => {
      var result = mxPopupMenuAddItem.apply(this, [title, image, funct, parent, iconCls, enabled])
